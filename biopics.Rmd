...
For this project, we will be using the `tidyverse` tool for data analysis. This is an R package that consists of tools such as `ggplot2`(data visualization),`tidyr`(data modification), `dplyr` (data management). The data set is obtained by calling the `fivethirtyeight` package using the command `library(fivethirtyeight)`.
...
### How many biopics have there been potraying each gender?
...
Most of the biopics depict white americans followed by african americans. There are little to no representation for
